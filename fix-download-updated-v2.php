...
function fix_download_process($url, $format_key, $config) {
    ...
    // If it's a YouTube URL, use the specialized direct bypass function
    if ($is_youtube) {
        require_once('youtube-direct-bypass.php');
        youtube_direct_bypass($url, $format_key, $config);
        exit;
    }
                ...
                if ($return_var4 === 0 && !empty($output4[0]) && filter_var($output4[0], FILTER_VALIDATE_URL)) {
                    ...
                    if (function_exists('curl_init')) {
                        ...
                        if (empty($curl_error) && file_exists($temp_file) && filesize($temp_file) > 1000) {
                            ...
                            debug_log("Direct URL download failed: $curl_error", $config);
                        }
                    } else {
